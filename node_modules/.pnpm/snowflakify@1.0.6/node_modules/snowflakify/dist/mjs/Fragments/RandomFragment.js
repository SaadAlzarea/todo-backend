...
export default class RandomFragment extends FragmentBase {
    ...
    constructor(bits, fn) {
        ...
        if (fn && !['number', 'bigint'].includes(typeof fn()))
            throw new TypeError(`[RND_FUNCTION_RETURN_TYPE]: RandomFragment custom function return is of type: ${typeof fn()}; Expected number or bigint.`);
    }
    getValue() {
        if (this.fn) {
            const rndNum = BigInt(this.fn());
            if (rndNum < 0n)
                throw new TypeError(`[RND_FUNCTION_BAD_RETURN]: RandomFragment custom function returned a negative value.`);
            if (rndNum >= 1n << BigInt(this.bits))
                throw new TypeError(`[RND_FUNCTION_BAD_RETURN]: RandomFragment custom function returned a value bigger than (2 ** ${this.bits}) - 1.`);
            return rndNum;
        }
        if (this.bits <= 47)
...
//# sourceMappingURL=RandomFragment.js.map