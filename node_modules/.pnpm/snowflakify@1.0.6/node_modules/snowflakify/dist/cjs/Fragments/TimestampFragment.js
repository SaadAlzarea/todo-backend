 ...
 * @public
 */
class TimestampFragment extends FragmentBase_js_1.default {
    /**
    ...
    constructor(bits, epoch = DEFAULT_EPOCH) {
        super(bits);
        if (bits < 38)
            throw new RangeError('[TIMESTAMP_BITS_INVALID_RANGE]: TimestampFragment bits must be greater than or equal to 38');
        if (typeof epoch !== 'number')
            throw new TypeError('[EPOCH_INVALID_TYPE]: TimestampFragment epoch must be a number.');
        if (epoch < 0 || epoch > Date.now())
            throw new RangeError('[EPOCH_INVALID_RANGE]: TimestampFragment epoch must be within 0 and Date.now() at instantiation time.');
        if (bits >= 58) {
        ...
        const firstHrTime = process_1.hrtime.bigint();
        const unixMilliseconds = Date.now();
        const secondHrTime = process_1.hrtime.bigint();
        this.nanoTimeAnchor =
    ...
    getValue() {
        ...
        if (this.sequenceFragmentRef)
            this.checkForSequenceCollision();
     ...
     */
    unixNow() {
        return (this.nanoTimeAnchor + process_1.hrtime.bigint()) / this.timeUnit;
    }
    /**
    ...
    checkForSequenceCollision() {
        ...
        if (this.sequenceFragmentRef.willReset())
            return this.waitForNextTimestamp();
    }
}
exports.default = TimestampFragment;
